...
if [ -d $DIR1 ]; then
	...
	FileList=`ls $DIR1*.dat`
	for file in ${FileList[@]};do
		SIZE=`ls -alh $file | awk '{print $5}'`
	done
	printf "%-30s %-8s %-8s\n" $DIR1 $num $SIZE
fi
if [ -d $DIR2 ]; then
	num=$(ls -l $DIR2 | grep "dat" | wc -l)
	FileList=`ls $DIR2*.dat`
	for file in ${FileList[@]};do
		SIZE=`ls -alh $file | awk '{print $5}'`
	done
	printf "%-30s %-8s %-8s\n" $DIR2 $num $SIZE
